...
class PlayScraperTest(unittest.TestCase):
    ...
    def test_invalid_language_code_raises(self):
        ...
        self.assertEqual(
            "invalid is not a valid language interface code.", str(e.exception)
        )
    ...
    def test_invalid_geolocation_code_raises(self):
        ...
        self.assertEqual(
            "invalid is not a valid geolocation country code.", str(e.exception)
        )
...
class DetailsTest(ScraperTestBase):
    def test_fetching_app_with_all_details(self):
        ...
        self.assertTrue(
            ...
            and all(
                x is not None and x.startswith("https://")
        ...
        for k, v in app_data.items():
            self.assertTrue(
                isinstance(v, (basestring, bool, dict, int, list, type(None)))
            )
...
class CollectionTest(ScraperTestBase):
    ...
    def test_detailed_collection_different_language(self):
        ...
        # hardcode an English mapping for the various additional info section
        # titles.
        self.assertTrue(all([apps[0][x] is None for x in ADDITIONAL_INFO_KEYS]))
    ...
    def test_promotion_collection_id(self):
        apps = self.s.collection(
            "promotion_3000000d51_pre_registration_games", results=2
        )
    ...
    def test_invalid_collection_id(self):
        ...
        self.assertEqual(
            "Invalid collection_id 'invalid_collection_id'.", str(e.exception)
        )

    def test_invalid_category_id(self):
        with self.assertRaises(ValueError) as e:
            self.s.collection("TOP_PAID", "invalid_category_id")
        self.assertEqual("Invalid category_id 'invalid_category_id'.", str(e.exception))

    def test_invalid_num_results_over_120(self):
        with self.assertRaises(ValueError) as e:
            self.s.collection("TOP_PAID", "LIFESTYLE", results=121)
        self.assertEqual("Number of results cannot be more than 120.", str(e.exception))

    def test_invalid_page_x_results_over_500(self):
        with self.assertRaises(ValueError) as e:
            self.s.collection("TOP_PAID", "LIFESTYLE", results=120, page=5)
        self.assertEqual(
            "Start (page * results) cannot be greater than 500.", str(e.exception)
        )
...
class SearchTest(ScraperTestBase):
    ...
    def test_page_out_of_range_not_between_0_and_12(self):
        ...
        self.assertEqual(
            "Parameter 'page' (13) must be between 0 and 12.", str(e.exception)
        )
